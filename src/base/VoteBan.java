...
public class VoteBan {
    ...
    static long parseTime(String time) throws NumberFormatException {
        String t = time.replaceAll("\\s+", "").replaceFirst("(\\d+d)", "P$1T");
        t = t.charAt(0) != 'P' ? "PT" + t.replace("min", "m")
                : t.replace("min", "m");
        Duration dur = Duration.parse(time);
        if (dur == null) {
            return Mathf.clamp(Long.parseLong(time), 0, 5256000);
        }
    ...
    public void newBan(String uuid, String ip, long minutes, String reason, Player voterPly) {
        ...
        String msg = " for [scarlet]" + Base.formatTime(minutes) + "[accent] minutes\nReason: [white]"
                + reason;
        if (voterPly.admin) {
    ...
    public String startVoteBan(String uuid, long minutes, String reason, String voter) {
        // Assumes:
        ...
        if (!voterPly.admin) {
            ...
            if (minutes > max) {
                return String.format("[accent]Max ban time for your rank is [scarlet]%d [accent]minutes", max)
                        + votekickSyntax;
            }
        }
